/* Kerbyggð — the Hótel Holt engine, unchanged except where the content is
...
/* SMOOTH decides whether the lerp scroller, the gravity wells and the snap run
   at all. It used to be a const evaluated once at import: a window that loaded
   under 768px and was then widened kept `no-scroll-smooth` forever, so the page
   scrolled natively, the wells never paced the pinned chapters, and the whole
   design read as "the motion is missing". Same family as the innerWidth bug
   above — a breakpoint answered once is a breakpoint that goes stale. It is now
   re-read whenever the query changes, and the scroller is resynced to wherever
   the native scroll actually is so the flip cannot jump the page. */
let SMOOTH = !isTouch && !reduced && mdUp();
const applySmooth = () => {
  html.classList.toggle('has-scroll-smooth', SMOOTH);
  html.classList.toggle('no-scroll-smooth', !SMOOTH);
};
applySmooth();
for (const q of ['(min-width: 768px)', '(prefers-reduced-motion: reduce)']) {
  matchMedia(q).addEventListener('change', () => {
    const next = !matchMedia('(hover: none) and (pointer: coarse)').matches
      && !matchMedia('(prefers-reduced-motion: reduce)').matches && mdUp();
    if (next === SMOOTH) return;
    SMOOTH = next;
    applySmooth();
    // hand the scroller the real position before it starts driving again
    if (typeof scroller !== 'undefined' && scroller) { scroller.y = scroller.target = window.scrollY; }
...
/* custom scrollbar */
(() => {
  /* Bound unconditionally, guarded per frame instead of at import: with an
     early return the scrollbar stayed dead for the rest of the session on any
     window that started narrow, even after SMOOTH came back. */
...
const PATTERNS = {
  ...
  landingLocationMapDesktop: { measure: 'closest:.sticky', keys: () => [
      { v: 100, e: 0, p: { transform: 'translate(-50%,-50%) translate(-21.7%,-53%) scale(3.05)' } },
      { v: 0, e: 0, p: { transform: 'translate(-50%,-50%) translate(-21.7%,-23.3%) scale(3.05)' } },
      { v: -100, e: 0, p: { transform: 'translate(-50%,-50%) translate(-23.9%,10.3%) scale(2.3)' } },
      { v: -450, e: 0, p: { transform: 'translate(-50%,-50%) translate(-23.9%,10.3%) scale(2.3)' }, easing: 'easeSection' },
...
const preloader = (() => {
  ...
  const skip = !mdUp() || location.hash === '#skip-preloader' || sessionStorage.getItem('kerb-preloaded');
...
const loadTex = (name, w = 1200) => {
  ...
  const attempt = i => new Promise(res => {
    if (i >= widths.length) { console.warn('[kerb] no texture for', name); return res(null); }
...
const rooms = (() => {
  ...
  let raycaster, ndc;
  const loader = chapterLoader($('.l-three-worlds-webgl'), 'Loading the houses');
  ...
  const go = i => { const r = ROOMS[i]; if (r && r.href) (window.__kerbNav || (h => { location.href = h; }))(r.href); };
...
const booking = (() => {
  ...
  const build = () => {
    ...
    el.innerHTML = `
      <div class="modal__bg"><img class="modal__img" src="assets/img/paint-house-dusk@1440.webp" alt=""></div>
      <div class="modal__scroller bk">
        <div class="bk__inner" role="dialog" aria-modal="true" aria-labelledby="bk-h">
          <div class="bk__head">
            <p class="bk__eyebrow text-small">SNDR Booking · demonstration</p>
            <h2 class="bk__title" id="bk-h"><span class="text-subtitle">Book</span><span class="h2">DIRECTLY</span></h2>
...
<!-- No close button of its own: the header's X is fixed, always on screen and
    ...
    const nights = () => {
      ...
      $('[data-nights]', el).textContent = n > 0
        ? `${n} night${n > 1 ? 's' : ''} · breakfast from seven`
    ...
    form.addEventListener('submit', (e) => {
      ...
      $('[data-done-body]', el).textContent =
        `In the real engine the front desk would now have a request for the ${r.toLowerCase()}, ${g} guest${g > 1 ? 's' : ''}, ${n} night${n > 1 ? 's' : ''}, and could confirm it from their phone. This preview sends nothing.`;
...
const startPage = () => {
  ...
  sessionStorage.setItem('kerb-preloaded', '1');
  measureAll();
  // header + reveals
...
const pageTransition = (() => {
  ...
  window.__kerbNav = raise;                                       // the WebGL ring navigates through the same curtain
